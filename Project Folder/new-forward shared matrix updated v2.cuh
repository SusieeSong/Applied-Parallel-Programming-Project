...
__global__ void MatrixMultiplyKernel(const int H_unroll, const int M, const int W_unroll, float * x_unroll, float * w, float * y){
    ...
    __shared__ float w_tile1[TILE_WIDTH][TILE_WIDTH], x_tile1[TILE_WIDTH][TILE_WIDTH];
    __shared__ float w_tile2[TILE_WIDTH][TILE_WIDTH], x_tile2[TILE_WIDTH][TILE_WIDTH];
    ...
    float (*w_tile)[TILE_WIDTH] = w_tile1;
    float (*w_tile_compute)[TILE_WIDTH] = w_tile2;

    float (*x_tile)[TILE_WIDTH] = x_tile1;
    float (*x_tile_compute)[TILE_WIDTH] = x_tile2;
    ...
    for(ph = 0; ph < ((H_unroll - 1) / TILE_WIDTH + 1); ph++){
        if(Row < M && (ph * TILE_WIDTH + tx) < H_unroll)
            w_tile[ty][tx] = w2d(Row, ph * TILE_WIDTH + tx);
        
        if((ph * TILE_WIDTH + ty) < H_unroll && Col < W_unroll)
            x_tile[ty][tx] = x3d_unroll(b, ph * TILE_WIDTH + ty, Col);
        ...
        w_temp = w_tile;
        w_tile = w_tile_compute;
        w_tile_compute = w_temp;

        x_temp = x_tile;
        x_tile = x_tile_compute;
        x_tile_compute = x_temp;



        for(i = 0; i < TILE_WIDTH; ++i){
            CValue += w_tile_compute[ty][i] * x_tile_compute[i][tx];   
        }
