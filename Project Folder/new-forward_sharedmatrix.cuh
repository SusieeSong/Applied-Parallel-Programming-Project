...
__global__ void MatrixMultiplyKernel(const int H_unroll, const int M, const int W_unroll, float * x_unroll, float * w, float * y){
    ...
    __shared__ float w_tile[TILE_HEIGHT][TILE_HEIGHT], x_unroll_tile[TILE_HEIGHT][TILE_WIDTH];
    ...
    int Row = blockIdx.z * TILE_HEIGHT + ty;
    ...
    for(int ph = 0; ph < ((H_unroll - 1) / TILE_HEIGHT + 1); ph++){
        if(Row < M && (ph * TILE_HEIGHT + tx) < H_unroll && tx < TILE_HEIGHT)
            w_tile[ty][tx] = w2d(Row, ph * TILE_HEIGHT + tx);
        if((ph * TILE_HEIGHT + ty) < H_unroll && Col < W_unroll)  
            x_unroll_tile[ty][tx] = x3d_unroll(b, ph * TILE_HEIGHT + ty, Col);     
        __syncthreads();

        for(int i = 0; i < TILE_HEIGHT; ++i){
            if(Row < M && (ph * TILE_HEIGHT + i) < H_unroll && (ph * TILE_HEIGHT + i) < H_unroll && Col < W_unroll)
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w) {
    ...
    dim3 Matrix_gridDim(B, ceil(W_unroll/(float)TILE_WIDTH), ceil(M/(float)TILE_HEIGHT));
    dim3 Matrix_blockDim(TILE_WIDTH, TILE_HEIGHT, 1);
