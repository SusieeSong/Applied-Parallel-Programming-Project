...
__global__ void forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K) {
    ...
    */
    ...
    extern __shared__ float shmem[];
        //float *x_shared2d = &shmem[0];
        //float *k_shared2d = &shmem[X_tile_width*X_tile_width];
        
    #define x_shared2d(i1,i0) shmem[K*K + (i1)* (TILE_WIDTH + K -1) +i0]
    #define k_shared2d(i1,i0) shmem[(i1)*K +i0] 
        ...
        int c, p, q;
          ...
          for (int i=h0; i<X_tile_width; i+= TILE_WIDTH){
             for (int j=w0; j<X_tile_width; j+= TILE_WIDTH){
              if((h_base + i) < H && (w_base + j) < W)
              x_shared2d(i,j)=x4d(n, c, h_base + i, w_base + j);
              else 
                x_shared2d(i,j)=0;
        }
    }
         ...
         if ((h < H_out)&&(w < W_out)){
            for (p=0; p< K; p++){
                for (q=0; q< K; q++)
                    acc = acc+ x_shared2d(h0+p,w0+q) * k_shared2d(p,q);
            }
            y4d(n,m,h,w) =acc;
        }
         __syncthreads();
