...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (row < dim){
    ...
    {
      dot += matData[matColStart[sec]+row] * vec[matCols[matColStart[sec]+row]]; //assume matrows[0]=3;matrows[1]=1;matrows[2]=0;sec=0-thread0 and 1 executes; when sec=1-- only thread0
      //sec++;
  }
